...
router
	...
	.post(async (req, res) => {
		...
		try {
			...
			formData.colorInput = validation.checkStringArray(
				formData.colorInput,
				"Color",
				1
			);
		...
		try {
			...
			formData.breedInput = validation.checkStringArray(
				formData.breedInput,
				"Breeds",
				1
			);
		...
		try {
			...
			formData.weightInput = validation.checkWeight(
				formData.weightInput,
				"Weight"
			);
		} catch (error) {
			res.status(400).render("error", { title: "Add Dog Error", error: error });
		...
		try {
			formData.descriptionInput = validation.checkString(
		...
		try {
			...
			formData.traitInput = validation.checkStringArray(
				formData.traitInput,
				"Traits",
				0
			);
		...
		try {
			...
			formData.medicalInput = validation.checkStringArray(
				formData.medicalInput,
				"Medical Info",
				0
			);
		...
		try {
			...
			formData.vaccineInput = validation.checkStringArray(
				formData.vaccineInput,
				"Vaccines",
				0
			);
		...
		try {
			let newDog = await dogData.addDog(
				formData.nameInput,
				formData.sexInput,
				formData.ageInput,
				formData.colorInput,
...
router.route("/:dogId/delete").delete(async (req, res) => {
	/*  Delete
                -Delete dog
        */
	let signedIn = true;

	...
	.post(async (req, res) => {
		...
		if (!dog || Object.keys(dog).length != 12) {
			return res
				...
				.render("error", {
					title: "Error Page",
					error: "There are fields missing in the request body",
				});
		}
		try {
			...
			band.description = validation.checkString(
				band.description,
				"Description"
			);
			band.traits = validation.checkStringArray(band.traits, "Traits", 0);
			band.medicalInfo = validation.checkStringArray(
				band.medicalInfo,
				"Medical Info",
				0
			);
			...
			return res.status(400).render("error", { title: "Invalid Input" });
		}
		const {
			name,
			sex,
		...
		const newDog = await dogData.create(
			name,
			sex,
			age,
			color,
		...
		return res.status(200).render("posts", { title: "New Post", dog: newDog });
	});

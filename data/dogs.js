...
const exportedMethods = {
	...
	async updateDog(
		...
		for (let i = 0; i < allDogs.length; i++) {
			if (
				...
				allDogs[i].sex === sex &&
				allDogs[i].age === age &&
				allDogs[i].color === color &&
				allDogs[i].breeds === breeds &&
				allDogs[i].weight === weight &&
				allDogs[i].description === description &&
		...
		if (updatedInfo.lastErrorObject.n === 0)
			throw [
				404,
				`Error: Update failed! Could not update post with id ${dogId}`,
			];
	...
	async addLike(dogId, userId) {
		...
		for (let i in myUser.liked) {
			if (myUser.liked[i] === dogId)
				throw `Error: Cannot like the same dog twice`;
		}
		let updatedDog = {
			name: myDog.name,
			sex: myDog.sex,
		...
		if (updatedInfoDog.lastErrorObject.n === 0)
			throw [
				404,
				`Error: Update failed! Could not update post with id ${dogId}`,
			];
	...
	async removeLike(dogId, userId) {
		...
		let flag = false;
		myUser.liked.forEach((element) => {
			if (element === dogId) {
				flag = true;
			}
		});
		if (flag === false) {
			throw "Error: Cannot unlike a dog that has not been liked";
		}
		...
		return `${myDog.name} has been successfully unliked!`;
	},
	...
	async removeDog(id) {
		...
		if (updatedInfoUser.lastErrorObject.n === 0)
			throw [
				404,
				`Error: Update failed! Could not update post with id ${myDog.userId}`,
			];

		const dogCollection = await dogs();
		const deletedDog = await dogCollection.findOneAndDelete({
			_id: new ObjectId(id),
		});
		...
		//now we have to remove this dogs from the pending, likes, and dislike of everyone else
	...
	async getMyDogs(id) {
		...
		for (let i = 0; i < myDogIDs.length; i++) {
			...
			const curDog = await dogCollection.findOne({
				_id: new ObjectId(curDogID),
			});
